<template>
  <div class="home" v-if="table.action_id">
    <div class="top">
      <div class="back"></div>
            ...
            <li v-for="(food,i) in item.category_goods" :key="i" class="rightLi" @click="goDetail(food.goods_id)">
              <div class="rowDiv" :itemId="food.items[0].item_id">
                ...
                <div class="info">
                  ...
                  <div class="sell" v-if="food.item_minimum && food.item_minimum > 1">
                    起订量
                    <span>{{food.item_minimum}}</span>
...
export default {
  ...
  methods: {
      ...
      tableDetail({ table_id: this.table_id }).then(res => {
        ...
        if( !table.action_id ){
          return this.$router.push(
            `/orderHome/${this.store_id}/${this.table_id}`
          );
        }
    ...
    goSearch() {
      this.$router.push(`/homeSearch/${this.store_id}/${this.table_id}`);
    },
    goDetail( goods_id ){
      this.$router.push(`/homeGoods/${goods_id}/${this.table_id}`)
    },
